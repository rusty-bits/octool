...
pub fn display_value(
    ...
    match oc_plist {
        ...
        Value::Boolean(v) => {
            match v {
                true => write!(&*screen, "{}: {}", style(key).green(), v).unwrap(),
                false => write!(&*screen, "{}: {}", style(key).red(), v).unwrap(),
            };
            if live_item {
                position.is_bool = true;
            }
        }
        Value::Data(v) => {
            write!(
        ...
        Value::Integer(v) => {
            write!(&*screen, "{}: {}{}", style(key).blue(), ls, v).unwrap();
        }
        Value::String(v) => {
